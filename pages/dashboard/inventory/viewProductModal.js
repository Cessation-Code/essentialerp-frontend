...
const ViewProductModal = ({ isOpen, onClose, selectedRowData }) => {
  ...
  async function handleDelete() {
    ...
    try {
      const response = await fetch(
        "http://essential-erp-10cac5b0da28.herokuapp.com/api/v1/product/deleteProduct",
        {
          method: "POST",
          headers: {
  ...
  async function handleEdit() {
    ...
    try {
      const response = await fetch(
        "https://essential-erp-10cac5b0da28.herokuapp.com/api/v1/product/editProduct",
        {
          method: "POST",
          headers: {
            "Access-Control-Allow-Origin": "*",
  ...
  return (
    <Modal header="Detailed View" closeModal={closeModal}>
      <form onSubmit={handleSubmit}>
        ...
        {/* error message */}
        {error && (
          <div className="flex flex-row pt-1 text-xs font-semibold justify-center text-red-600">
            {error}
          </div>
        )}
        ...
        {!isLoading && (
          <div className="flex flex-row justify-end gap-3">
            {!isEditMode && (
              <button
                type="reset"
                className="btn-sm bg-red-500 hover:bg-red-700 rounded text-white font-semibold text-sm py-1 px-4 mt-4"
                onClick={() => {
